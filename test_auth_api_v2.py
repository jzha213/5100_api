...
def test_user_register():
    ...
    try:
        ...
        register_data = {
            'username': f'testuser_auth_{random_num}',
            'password': 'test123456',
            'email': f'testuser_auth_{random_num}@example.com',
            'phone': f'13900{random_num}',
            'nickname': f'测试认证用户{random_num}'
        }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if profile_response.status_code == 200:
                print(f"✅ Token认证测试通过")
                print(f"   成功使用token访问受保护的API")
                return True
            else:
                print(f"❌ Token认证失败")
                return False
        else:
            print(f"❌ 无法获取token进行测试")
            return False
            
    except Exception as e:
        print(f"❌ Token认证测试异常: {e}")
        return False

def main():
    """主测试函数"""
    print("🚀 开始测试5100订水用户认证API (版本2)...")
    print("=" * 60)
    ...
    # 测试用户登录
    access_token = test_user_login(username) if username else test_user_login('testuser_auth')
    ...
    # 测试token认证
    token_auth_success = test_token_authentication()
    ...
    print(f"   - Token认证: {'✅' if token_auth_success else '❌'}")
    
    total_tests = 6
    passed_tests = sum([register_success, bool(access_token), profile_success, 
                       bool(superuser_token), invalid_login_success, token_auth_success])
